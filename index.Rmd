...
The remaining training dataset still has quite some predictors (52 in total). Given the nature of the measurements (e.g. acceleration in the x, y and z-direction when performing a certain action, which can be expected to be strongly correlated), it only seems logical to try to further reduce this data. We do this using principal component analysis. To see if the data is suitable for this, we first plot all predictors (because the grid.arrange function seems to be a bit unstable with a large number of graphs, we commented this code out)
...
Regarding our modeling strategy, our preferred modeling strategy would be one of stacking (i.e. estimating 3 different models, stacking the resulting predictions together and use the resulting data set to predict the outcomes of the testing data). Unfortunately, this is *not* possible due to the lack of the outcome variable in the testing data. We therefore use the random forest, which according to the lecture is one of the most accurate classification methods (together with boosting). An important consideration of working with large data sets is modeling speed. We took two measures to speed-up model estimation. The first one is changing the default bootstrapping for cross-validation to repeated k-fold cross validaion. Not only are the lecture slides a bit ambivalent as to the use of bootstrapping (see the corresponding slides), repeated k-fold cross validation is also advised to use with random forests (see e.g. https://bit.ly/3ph4jqS) as it reduces both bias and variance (another topic that seems to be highly debated, but that is for a different assignment). 
...
Now we fit the model (and have a coffee, as the train-command below takes about 20 minutes. It probably could be shorter but it works like a charm and I live my live slowly anyway):
...
We can assess the quality of our model by looking at the confusion matrix (using the training data, as the testing data does not contain the classe variable. The confusion matrix using the training data is based on a comparison of the modeled data to th held-out folds):

```{r confusion}
confusionMatrix.train(mod.rf)
```

The reported accuracy of the model is very high (close to 100%), which means that the model suits the training data well (in other words: the off-diagonal percentages are very low). We can also expect that this model is a little tuned to the noise in this training data, which implies that it will perform less well for the test data. Hence, the out-of-sample error for this test data will be larger. We did manage to get a 100% score for the prediction quiz, though. Although this does not say anything about the accuracy of our model (it probably is because the answers in the quiz are based on the predicted value, *not* on the observed values) it at least suggest this is the type of model the instructors envisioned when making this assignment. Cool stuff! :)