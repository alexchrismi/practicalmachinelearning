...
mat <- table(mod.val, validation$classe) # confusion matrix
``` 

We then calculate the accuracy of this model:

``` {r accuracy}
round(sum(diag(mat) / sum(mat)), 2)
```

From this confusion matrix -as well as a comparison of the reported accuracies, which for the training data is based on a comparison of the modelled data to the held-out folds- we believe that the model makes a reasonable prediction of the classe-variable in the validation data set. The accuracy of the model applied to the training data is a bit higher. This is because the model is a little tuned to the noise in this training data, which implies that it will perform less well for the testing data and hence, the out-of-sample error for the validation set is larger (and with that its accuracy is lower).We did manage to get a 100% score for the prediction quiz, though. Although this does not say anything about our model's accuracy (it probably is because the answers in the quiz are based on the predicted value, *not* on the observed values), it at least suggests this is the type of model the instructors envisioned when making this assignment. Cool stuff! :)